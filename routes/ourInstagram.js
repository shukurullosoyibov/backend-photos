...
import InstagrasModals from '../models/ourInstagram.js'
...
router.post('/upload', upload.single('image'), async (req, res) => {
  try {
    ...
    const file = new InstagrasModals({
      filename: filename,
...
router.get('/all', async (req, res) => {
    try {
        const data = await InstagrasModals.find();
...
router.get('/:id', async (req, res) => {
  const { id } = req.params;
    try {
        const data = await InstagrasModals.findById(id);
        res.status(200).json({ message: 'success', data });
    } catch (error) {
          res.status(404).json({ message: 'error', error });
    }   
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    const deletedFile = await InstagrasModals.findByIdAndDelete(id);
  if(deletedFile){
...
router.get('/category/:slug', async (req, res) => {
  ...
  try {
    const photosByCategory = await InstagrasModals.find({ categorySlug: slug });
...
router.put('/:id', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    const existingFile = await InstagrasModals.findById(id);

    // Update file details in the database
    const updatedFile = await InstagrasModals.findByIdAndUpdate(
      id,
      {
